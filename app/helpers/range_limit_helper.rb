...
module RangeLimitHelper
  ...
  def render_range_input(solr_field, type, input_label = nil, maxlength = 4)
    ...
    default = params['range'][solr_field][type] if params['range'] &&
        params['range'][solr_field] &&
        params['range'][solr_field][type]

    html = label_tag("range[#{solr_field}][#{type}]", input_label, class: 'sr-only') if input_label.present?
    html + text_field_tag("range[#{solr_field}][#{type}]", default,
                           maxlength: maxlength, class: "form-control range_#{type}")
  end
  ...
  def range_display(solr_field, my_params = params)
    return '' unless my_params[:range][solr_field]['begin'] && my_params[:range][solr_field]['end']

    return BlacklightRangeLimit.labels[:missing] if my_params[:range][solr_field]['missing']

    hash = my_params[:range][solr_field]
    if hash['begin'] == hash['end']
      return sanitize "<span class='single'>#{h(hash['begin'])}</span>"
    else
      return sanitize "<span class='from'>#{h(hash['begin'])}</span> to <span class='to'>#{h(hash['end'])}</span>"
    end
  end
