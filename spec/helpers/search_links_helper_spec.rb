...
RSpec.describe SearchLinksHelper do
  describe '#other_subjectify' do
    ...
    it 'provides links to general subject search based on the given other subjects' do
      ...
      expect(full_subject).to eq('<ul><li><a class="search-subject" title="Check catalog for: Power Amplifiers" ' \
                                 ...
                                 '<a class="search-subject" ' \
                                 'title="Check catalog for: Fluid Mechanics and Thermodynamics" ' \
  ...
  describe '#series_links' do
    ...
    context 'when there is a series value but no strict version of the series title' do
      ...
      it 'assembles the link correctly' do
        link = series_links link_doc
        link_expect = '<ul><li><a class="search-series" ' \
                      'title="Check catalog for: Lecture Notes in Electrical Engineering, ' \
    ...
    context 'when there are multiple series values and strict versions of the series title' do
      ...
      it 'assembles the link correctly' do
        ...
        link_expect = '<ul><li><a class="search-series" ' \
                      ...
                      'title="Check catalog for: Series Title 2" ' \
                      'href="/?search_field=series&amp;q=Series+Title+2">Series ' \
  ...
  describe '#title_links' do
    ...
    it 'assembles a link to title search and puts it in a list' do
      ...
      expect(links).to match '<ul><li><a title="Check catalog for: Some Title" ' \
                             'href="/?search_field=title&amp;q=Some+Title">Some Title</a></li><li>' \
                             '<a title="Check catalog for: Another Title" ' \
                             'href="/?search_field=title&amp;q=Another+Title">Another Title</a></li></ul>'
    end
  end
