...
RSpec.describe 'Author Browse', type: :feature do
  context 'when starting at the beginning' do
    specify do
      visit author_browse_path

      expect(page).to have_selector('h1', text: 'Browse by Author')
  ...
  context 'when the search prefix is lowercased' do
    specify do
      ...
      expect(page).not_to have_link('Steinem, Gloria', href: /all_authors_facet/)
    end
  end
end
