...
RSpec.describe 'Hathi Links', :vcr, type: :feature, js: true do
  before do
    allow_any_instance_of(ExternalLinks::HathiLinksComponent).to receive(:search_item).and_return 'oclc/12345'
  end
  ...
  context 'when nothing is returned from HathiTrust' do
    it 'does not display Hathi Link' do
      ...
      expect(page).to have_css("img[src*='HathiTrust_logo.svg']", visible: :hidden)
        .and have_css("div[data-search-item='oclc/12345']", visible: :hidden)
    end
  end
