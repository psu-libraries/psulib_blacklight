...
RSpec.describe 'Facet Queries' do
  ...
  context 'when search volume exceeds limit' do
    context 'when AUTH_FOR_QUERIES is false' do
      before do
        ENV['AUTH_FOR_QUERIES'] = 'false'
      end

      after do
        ENV['AUTH_FOR_QUERIES'] = nil
      end

      it 'does not require auth for multiple queries' do
    ...
    context 'when AUTH_FOR_QUERIES is true' do
      before do
        ENV['AUTH_FOR_QUERIES'] = 'true'
      end
