...
RSpec.describe 'Single Item Browse Button', type: :feature do
  context 'when record has one LC call number', js: true do
    before do
      visit '/catalog/22080733'
    end

    it 'displays browse button for LC call number' do
      expect(page).not_to have_selector('button[data-toggle=dropdown]')
      expect(page).to have_link('Browse Nearby on Shelf',
                                href: '/browse/call_numbers?classification=lc&nearby=PS3610.O63A84+2018')
    end
  end

  context 'when record has multiple LC call numbers', js: true do
    before do
      visit '/catalog/53953'
    end

    it 'displays browse button for LC call numbers' do
      expect(page).to have_selector('button[data-toggle=dropdown]')
  ...
  context 'when record has one Dewey call number', js: true do
    before do
  ...
  context 'when record has multiple Dewey call numbers', js: true do
    before do
  ...
  context 'when record has multiple LC and Dewey call numbers', js: true do
    before do
